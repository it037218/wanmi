...
$(document).ready( function () {
	$('#btn1').click ( function () {
	    $.ajax({
		    url: '<?php echo $domain;?>/redbag/getRedBag',
		    type: 'POST',
		    data:'code=' + $('#code').val()+'&phone='+$('#account').val(),
		    dataType: 'json',
		    timeout: 3000,
		    error: function(){
		    ...
		    success: function(data){
		    	if(data.error == 0){
		    		window.location='<?php echo $domain;?>/redbag/showRedbagDetail?code='+data.code+'&account='+data.account+'&money='+data.money+'&isnew='+data.isnew;
		        }else if(data.error == 1){
			        if(data.msg=='红包已领完'){
			        	window.location.reload();
				    }else{
			      	 $("#tishi_msg").html(data.msg);
					 showBg();	
						}
		        }else if(data.error == 2){
			      	 $("#tishi_msg").html(data.msg+'！</br>'+data.data.money+'元红包已存入您的易米融账户！');
					 showBg();	
			    }
		    }
	    });
...
<div id="main_page" style="background: url('<?php echo $static_domain; ?>/images/bg/hongb01.jpg');background-size:cover;background-repeat:no-repeat;width: 100%"> 
	<div style="height: 30%;text-align: center;font-size: 36px">
		<?php if($error==1){?>
			<div style="font-size: 60px;padding-top: 60px;"><img alt="" src="<?php echo $static_domain; ?>/images/bg/hongb03.png"></div>
		<?php }?>
		<?php if($error==0){?>
			<div style="height: 60px"></div>
			<div style="font-size: 80px"><img alt="" src="<?php echo $static_domain; ?>/images/bg/hongb02.png"></div>
		<?php }?>
	</div>
	<div style="height: 35%">
		<div style="text-align: center;font-size: 70px;padding-top: 170px;color:#ffee95;visibility: hidden;"><font style="font-size: 140px">20</font>元</div>
	</div>
	<div class="login" id="login1" style="background: none;height: 20%">
		<ul style="margin: 15% 5% 0 5%">
		<li>
			<input class="radius" type="tel" id="account" name="account" placeholder="输入手机号码,领取红包" style="border-radius:1em;border: 1px;height: 85px;font-size: 40px;">
		</li>
		<li class="nextBtn">
			<?php if($error==1){?>
				<button class="btn1" id="btn2" style="color:white;background: grey;font-size: 34px;border-radius:1em;height: 85px;">立刻开抢</button>
			<?php }?>
			<?php if($error==0){?>
				 <input name="btn1" class="btn1" id="btn1" type="button"  style="font-size:36px;color:#a51617 border:0; background:url('<?php echo $static_domain; ?>/images/bg/hongb_btn.png') left;" value='立刻开抢'/>
			<?php }?>
		</li>
		</ul>
	</div>
...
<div id="dialog"> 
	...
	<div style="background: url('<?php echo $static_domain; ?>/images/hd/dikuang.png');background-size:contain;background-repeat:no-repeat;height: 30%;width: 100%;height: 100%">
		...
		<div style="height: 80%;text-align: center;color: #711900;font-size: 36px">
			...
			<div style="height: 30%">
				<input type="button" value="立刻下载" style="background: url('<?php echo $static_domain; ?>/images/hd/anniu.png');background-size:contain;background-repeat:no-repeat;width: 70%;height: 70%;border: none;font-size: 32px;color:#f1eb88" onClick="window.location='http://a.app.qq.com/o/simple.jsp?pkgname=cn.app.cmibank'">
			</div>
		</div>
	</div>
</div>
