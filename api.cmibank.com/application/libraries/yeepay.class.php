...
class yeepay {
	...
	//回调接口
	private $callbackurl = 'http://119.29.74.53/yeepay_back.php';
	
	public function __construct($config) {
    	if(isset($_SERVER['SERVER_ADDR']) && $_SERVER['SERVER_ADDR'] == '127.0.0.1'){
            $this->callbackurl = 'http://120.132.69.210/yeepay_back.php';
        }else if(isset($_SERVER['SERVER_ADDR']) && $_SERVER['SERVER_ADDR'] == '117.50.2.20'){
            $this->callbackurl = 'http://120.132.69.210/yeepay_back.php';
        }else{
            $this->callbackurl = 'http://119.29.74.53/yeepay_back.php';
        }
		...
		//$this->directBindPayURL = 'https://ok.yeepay.com/payapi/api/tzt/pay/bind/reuqest';      //短信支付
		$this->directBindPayURL = 'https://ok.yeepay.com/payapi/api/tzt/directbindpay';           //无短支付
	 ...
	 * @return type
	 */
	public function bindBankcard($identityid, $requestid, $cardno, $idcardno, $username, $phone, $userip) {
		$query = array(
	...
	public function directPayment($orderid, $transtime, $amount, $productname, $identityid, $card_top, $card_last, $orderexpdate, $userip) {
         ...
         *      [orderid] => 120812150707174218530463915
         *      [phone] => 18019710907
         *      [smsconfirm] => 0
         *  )
		 */
	...
	public function queryOrder($orderid){
	    ...
	    //print_r($query);
	    return $this->get($this->getQueryOrderUrl(), $query);
	}
	...
	public function payClearData($startdate, $enddate) {
		...
		if ($this->http_info['http_code'] == 405) {
			...
			return array('msg' => '此接口不支持使用GET方法请求', 'error' => 1003);
		}
		return $data;
	}
	...
	protected function parseReturnData($data) {
		...
		if (array_key_exists('error_code', $return) && !array_key_exists('status', $return)) {
			...
			return array('msg' => $return['error_msg'], 'error' => $return['error_code']);
		}
...
// 		dump($query);
		...
		if ($this->http_info['http_code'] == 405) {
			...
			return array('msg' => '此接口不支持使用POST方法请求', 'error' => 1004);
		}
