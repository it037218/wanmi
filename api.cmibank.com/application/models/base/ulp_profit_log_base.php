...
class ulp_profit_log_base extends Basemodel{

    public $_table = 'cmibank.cmibank_ulp_profit_log_';
    
    public function add_ulp_profit_log($uid, $data){
        $table = $this->getTableIndex($uid, $this->_table);
        $this->insertDataSql($data, $table);
    }
    
    public function  _get_db_user_longproduct_profit($uid, $starttime, $endtime){
        $table = $this->getTableIndex($uid, $this->_table);
...
//         echo $sql;
    ...
    public function _get_user_longproduct_profit($uid, $starttime, $endtime){
        $key = _KEY_REDIS_SYSTEM_LONGPRODUCT_LOG_PROFIT_LIST_PREFIX_ . date('Ymd', $starttime) .'-'. date('Ymd', $endtime) .':'. $uid;
        //         echo $key;
        $self = $this;
        $return = $this->remember($key, 0 , function() use($self , $uid, $starttime, $endtime) {
            $user_longprofit_log = $self->_get_db_user_longproduct_profit($uid, $starttime, $endtime);
    ...
    public function get_all_longmoney_with_odate($odate){
        $sum_profit = 0;
        $odate = strtotime($odate);
        for($i = 0 ; $i <= 15; $i++){
            $sql = "SELECT sum(f_longmoney) as p_sum FROM " . $this->_table . $i . " WHERE time = '$odate'";
            $data = $this->executeSql($sql);
            $sum_profit += $data[0]['p_sum'];
        }
        return $sum_profit;
    ...
    public function get_ulplist_with_odate($index, $odate){
        $sql = "SELECT * FROM " . $this->_table . $index . " WHERE time = '$odate'";
        return $this->executeSql($sql);
    }
    
    public function update_ulp_data_by_id($index, $data, $where){
