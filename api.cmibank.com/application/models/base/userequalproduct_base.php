...
class userequalproduct_base extends Basemodel{

    private $_table = 'cmibank.cmibank_userequalproduct_';
 
    private $product_tpl;
    
    public function addUserEqualProductInfo($uid, $data){
     ...
     */
    public function _get_db_userEqualProduct($uid, $type = 0){
        $table = $this->getTableIndex($uid, $this->_table);
        $where = array('uid' => $uid);
        if($type == 0){
            $where['status'] = 0;
    ...
    //用户所有产品
    public function getAllEqualProductInfo($uid){
        $data = $this->_get_db_userEqualProduct($uid , 1);
        return $data;
    }
    
    
    public function getUserEqualProductInfo($uid){
        $key = _KEY_REDIS_USER_EQUALPRODUCT_PREFIX_ . $uid;
        $self = $this;
        $return = $this->remember($key, 0 , function() use($self , $uid) {
            $userproductInfo = $self->_get_db_userEqualProduct($uid);
    ...
    public function rebuildUserEqualProductInfo($uid){
        $key = _KEY_REDIS_USER_EQUALPRODUCT_PREFIX_ . $uid;
        self::$container['redis_app_w']->delete($key);
        return $this->getUserEqualProductInfo($uid);
    }
    
    public function updateUserEqualProductStatus($data, $where){
        if(empty($where['uid'])){
            return false;
        }
        $tableName = $this->getTableIndex($where['uid'], $this->_table);
        $this->updateDataSql($tableName, $data, $where);
        $this->rebuildUserEqualProductInfo($where['uid']);
        return true;
    }
    
    
    public function get_tpl_equalproduct_info($pid){
        if(!isset($this->product_tpl[$pid])){
            $this->load->model('base/equalproduct_base', 'equalproduct');
            $info = $this->equalproduct->getProductDetail($pid);
    ...
    //计算单条产品收益
    public function count_equalproduct_profit($user_product, $product_tpl){
        //用户购买的天数
...
//         }
    ...
    public function setProfitDetailCache($uid, $type, $data){
        if($type == 0){
            $key = _KEY_REDIS_USER_EQUAL_NOT_SQUARE_PROFIT_PREFIX_ . $uid . '_' . date('Y-m-d');
        }else{
            $key = _KEY_REDIS_USER_EQUAL_PROFIT_PREFIX_ . $uid . '_' . date('Y-m-d');
        }
...
//             echo strtotime($index) . '<br />' . json_encode($_data);
    ...
    public function getSumUserEqualProduct($odate){
        $sum_money = 0;
    ...
    public function getAllSumUserEqualProduct($odate){
        $sum_money = 0;
        $time = strtotime($odate);
        $end_time = $time + 86400;
        for($i = 0 ; $i <= 15; $i++){
