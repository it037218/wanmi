...
class Basemodel extends CI_Model {
	...
	public function getInstanceServer() {
		...
		# 初始化 redis_app 读 机器
		$container['redis_account_r'] = $container->share(function() use ($self) {
			return $self->getRedisServer('redis_account' , 'read');
		});

		# 初始化 redis_app 写 机器
		$container['redis_account_w'] = $container->share(function() use ($self) {
			return $self->getRedisServer('redis_account' , 'write');
		});
...
//         });
	...
	public function remember($key, $minutes, Closure $callback , $type = _REDIS_DATATYPE_STRING , $redisR= '' , $redisW = '') {
		...
		switch ($type) {
            case _REDIS_DATATYPE_STRING:
                $value = $callback();
                if($value) $redisW->save($key, $value , $minutes);
                break;
            case _REDIS_DATATYPE_HASH:
                $redisW->hashSet($key, $value = $callback());
                break;
            default:
                break;
		}
...
//         if(empty($where)){
//             echo 'NO WHERE:' . $this->DBW->last_query();
//         }
        return $this->DBW->affected_rows();
    }
