            ...
            ?>
                ...
                <td>
                    <?php if($value['type'] == 'changping'){?>
                        <?php if($current_pid != '--'){?>
                            <a href="<?php echo OP_DOMAIN;?>longproduct/detail?&pid=<?php echo $current_pid;?>" target="navtab" title="查看">查看</a>
                            <?php if($editable==1){?>
                                <?php if($nextpname != '--'){?>
                                    &nbsp;&nbsp;|&nbsp;&nbsp;
                                    <a href="<?php echo OP_DOMAIN;?>onlinelongproduct/changeindex?ptid=<?php echo $value['ptid']?>&odate=<?php echo $odate; ?>" target="navtab" title="上线产品调序">调序</a>
                                <?php }?> 
	                            &nbsp;&nbsp;|&nbsp;&nbsp;
	                            <a href="<?php echo OP_DOMAIN;?>onlinelongproduct/Soldout/<?php echo $value['ptid']?>/<?php echo $current_pid;?>/<?php echo $value['lplist'][0]['odate']; ?>" target="ajaxTodo" title="你真的要设置为售罄么？">售罄</a>
                        	<?php }?>
                        <?php }?> 
                    <?php }else{?>
                        <?php if($current_pid != '--'){?>
                            ...
                            <?php if($editable==1){?>
                                <?php if($nextpname != '--'){?>
                                    &nbsp;&nbsp;|&nbsp;&nbsp;
                                    <a href="<?php echo OP_DOMAIN;?>onlinelongproduct/yugao?ptid=<?php echo $value['ptid']?>&odate=<?php echo $odate; ?>" target="navtab" title="预告产品">调序</a>
                                 <?php }?> 
	                            &nbsp;&nbsp;|&nbsp;&nbsp;
	                            <a href="<?php echo OP_DOMAIN;?>longproduct/delLongproduct/<?php echo $current_pid?>" target="ajaxTodo" title="您真的要删除吗?">删除</a>
                         	<?php }?>
                         <?php }?> 
					<?php }?>
                </td>
