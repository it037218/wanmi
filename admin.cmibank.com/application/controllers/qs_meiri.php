 ...
 * 合同管理
 * * */
class qs_meiri extends Controller {
    ...
    public function index(){
        $flag = $this->op->checkUserAuthority('每日数据',$this->getSession('uid'));
        $data=array();
        if($flag == 0){
            ...
            $data['count'] = $count;
            $log = $this->op->actionData($this->getSession('name'), '每日数据汇总', '', '查看', $this->getIP(), $this->getSession('uid'));
            $this->load->view('/qs_log/v_index', $data);
        	}
    }
        ...
        if($flag == 0){
            ...
            $data['count'] = $count;
            $log = $this->op->actionData($this->getSession('name'), '每日数据汇总', '', '查看', $this->getIP(), $this->getSession('uid'));
            $this->load->view('/qs_log/v_export', $data);
        	}
    }
    public function exportAll(){
    	...
    	if($flag == 0){
    		...
    		$list = $this->qs_log->getList(null ,'odate desc');
    		$count = $this->qs_log->getCount();
    		$data = array();
    		$odate_list = array();
